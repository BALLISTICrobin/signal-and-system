...
class FourierSeries:
    ...
    def calculate_a0(self, N=1000):
        ...
        y = self.func(x) 
        integrated_value = np.trapz(y, x)                    
        a0 = (1 / (2 * self.L)) * integrated_value
    ...
    def calculate_an(self, n, N=1000):
        ...
        x = np.linspace(-self.L, self.L, N)  
        y = self.func(x)                     
        cosTerm = np.cos(n * np.pi * x / self.L)
        an = (1 / self.L) * np.trapz(y * cosTerm, x)
        self.an.append(an)  
    ...
    def calculate_bn(self, n, N=1000):
        ...
        x = np.linspace(-self.L, self.L, N)  
        y = self.func(x)                    
        sinTerm = np.sin(n * np.pi * x / self.L)
        bn = (1 / self.L) * np.trapz(y * sinTerm, x)
        self.bn.append(bn)  
    ...
    def approximate(self, x):
        ...
        for n in range(1, self.terms + 1):
            ...
            approximation += self.an[n-1] * np.cos(n * np.pi * x / self.L)
            approximation += self.bn[n-1] * np.sin(n * np.pi * x / self.L)
...
def target_function(x, function_type="square"):
    ...
    if function_type == "square":
        ...
        x_scaled = x / (2 * np.pi)
        cycleCount = np.floor(0.5 + x_scaled)
        fractionalPart = x_scaled - cycleCount
        sawtooth = 2 * fractionalPart
        return sawtooth
        ...
        cycleCount = np.floor(0.5 + x_scaled)
        fractionalPart = x_scaled - cycleCount
        sawtooth_like_wave = 2 * fractionalPart
        triangle_wave = np.abs(sawtooth_like_wave)
        triangle_wave = 2 * triangle_wave - 1
